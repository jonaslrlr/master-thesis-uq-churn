    ...
    def __init__(
        ...
        if shared_layers is None:
            ...
            self.shared = GLU_Block(
                input_dim, output_dim, first=True,
        ...
        if n_glu_independent == 0:
            ...
            self.specifics = GLU_Block(
                spec_input_dim, output_dim, first=is_first,
...
class GLU_Layer(torch.nn.Module):
    def __init__(self, input_dim, output_dim, fc=None,
        ...
        self.fc = fc if fc is not None else Linear(input_dim, 2 * output_dim, bias=False)
        initialize_glu(self.fc, input_dim, 2 * output_dim)

        self.bn = GBN(2 * output_dim, virtual_batch_size=virtual_batch_size, momentum=momentum)

        # dropout module
        self.drop = torch.nn.Dropout(dropout) if dropout and dropout > 0 else torch.nn.Identity()

    def forward(self, x):
        x = self.fc(x)
        x = self.bn(x)
        values = x[:, : self.output_dim]
        gates  = x[:, self.output_dim :]
        out = values * torch.sigmoid(gates)
        out = self.drop(out)
        return out
