...
#
# Loss is fixed to MSE Bayes risk (Sensoy Eq.5) — the standard EDL loss.
# CE Bayes risk (Eq.4) was removed: it tends to inflate evidence and
# compress the uncertainty range, defeating the purpose of EDL.
#
# Ablation groups:
#   A) Shallow head:  head_hidden_dim=0  (original paper reproduction)
#   B) Deep head:     head_hidden_dim>0  (nonlinear evidence modulation)
# ──────────────────────────────────────────────────────────────────────
    ...
    for kl_coef in KL_COEF_GRID:
        for anneal_epochs in ANNEAL_EPOCHS_GRID:
            for head_hidden_dim in HEAD_HIDDEN_DIM_GRID:
                ...
                tag = f"kl{kl_coef}_ann{anneal_epochs}_{head_label}"
